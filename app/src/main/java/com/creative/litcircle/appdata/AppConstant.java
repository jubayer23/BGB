package com.creative.litcircle.appdata;


/**
 * Created by comsol on 11/8/2015.
 */
public class AppConstant {

    public static final String TAG = AppController.TAG;

    public static final String APP_NAME = "BGB";

    public static final int REQUEST_CHECK_SETTINGS = 100;


    public static final String[] pillars_name = {"189-PS1", "190-PS3", "160-JS1", "110-JS2"};

    public static final String[] pillars_condition = {"Good", "Average", "Bad" ,"Broken" ,"Missing"};

    public static final String[] gps_interval = {"5","30", "40", "50", "60", "300", "900", "1800", "3600"};


    public static final String[] mission_type = {"Smuggling", "Human Trafficking", "BOP Internal" ,"Miscellaneous" ,"Others"};


    public static final String pillar_entry_new = "1";
    public static final String pillar_entry_update = "0";


    public static final String ADMIN_PASSWORD = "1127";
    public static String APP_UPDATE_URL = "";
    public static boolean isForceLogout = false;
}
